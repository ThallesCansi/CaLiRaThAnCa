...
gdjs.TopDownMovementRuntimeBehavior.prototype.doStepPreEvents = function (instanceContainer) {
  ...
  for (const topDownMovementHook of this._topDownMovementHooks) {
    if (topDownMovementHook.afterPositionUpdateTODO) {
...
class CornerSlider {
   ...
   */
  constructor(instanceContainer, behavior, topDownBehavior) {
    ...
    this.obstacleManager = getManager(instanceContainer);
   ...
   */
  separateFromObstacles = function () {
    const object = this.topDownBehavior.owner;
    const objectAABB = this.getHitBoxesAABB();
    const minX = objectAABB.min[0];
    const minY = objectAABB.min[1];
   ...
   */
  separateFrom = function (obstacleBehavior) {
    const objectAABB = this.getHitBoxesAABB();
    const minX = objectAABB.min[0];
    const minY = objectAABB.min[1];
    const maxX = objectAABB.max[0];
   ...
   */
  updatePotentialCollidingObjects(maxMovementLength) {
...
class ObstacleManager {
   ...
   */
  getAllObstaclesAround(aabb, maxMovementLength, result) {
...
class Obstacle {
   ...
   * @param {gdjs.RuntimeBehavior} behavior
   */
  constructor(instanceContainer, behavior) {
    this.oldX = 0;
    this.oldY = 0;
    this.oldWidth = 0;
    this.oldHeight = 0;
    ...
    this.behavior = behavior;
    this.manager = getManager(instanceContainer);
...
gdjs.__topDownCornerSlidingExtension = {
  CornerSlider,
  Obstacle,
};
...
gdjs.evtsExt__TopDownCornerSliding__DefineJavaScriptForTopDown.eventsList0(runtimeScene, eventsFunctionContext);
